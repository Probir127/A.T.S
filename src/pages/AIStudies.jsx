  ...
  return (
    <div className="bg-[#07080C] text-[var(--text-main)] min-h-screen pt-32 pb-24">
      ...
      <div className="max-w-[1200px] mx-auto px-[5%]">
        <div className="mb-16">
          <p className="text-[0.75rem] uppercase tracking-[4px] text-[var(--accent-primary)] mb-6 font-semibold font-['Inter']">Research Lab</p>
          <h1 className="font-['Space_Grotesk'] text-[clamp(3rem,8vw,5.5rem)] leading-none uppercase tracking-[1px] mb-6">
            AI <span className="text-[var(--accent-primary)]">Studies</span>
          </h1>
        ...
        <div className="flex flex-col gap-8">
          {studies.map((study, i) => (
            <article
              key={i}
              className="group p-8 md:p-12 rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] hover:border-[rgba(99,102,241,0.25)] transition-all duration-500 flex flex-col md:flex-row gap-8 items-start cursor-pointer"
            >
              <div className="w-16 h-16 rounded-2xl bg-[rgba(99,102,241,0.08)] text-[var(--accent-primary)] flex items-center justify-center text-3xl border border-[rgba(99,102,241,0.1)] flex-shrink-0">
                <i className={study.icon}></i>
              </div>
              <div className="flex flex-col gap-3">
                <span className="text-[0.7rem] uppercase tracking-[2px] font-bold text-[var(--accent-primary)]">{study.category}</span>
                <h2 className="font-['Space_Grotesk'] text-[clamp(1.5rem,3vw,2rem)] tracking-[0.5px] leading-tight group-hover:text-[var(--accent-primary)] transition-colors duration-300">
                  {study.title}
                </h2>
                <p className="text-[1.05rem] text-[var(--text-muted)] leading-[1.7] max-w-[700px]">{study.excerpt}</p>
                <span className="mt-2 text-[0.8rem] uppercase tracking-[2px] font-bold text-[var(--accent-primary)] group-hover:underline flex items-center gap-2">
