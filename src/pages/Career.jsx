  ...
  return (
    <div className="bg-[#07080C] text-[var(--text-main)] min-h-screen pt-32 pb-24">
      ...
      <div className="max-w-[1200px] mx-auto px-[5%]">
          ...
          {openRoles.length > 0 ? (
            <div className="flex flex-col gap-6">
              {openRoles.map((role, i) => (
                <div key={i} className="group p-8 md:p-10 rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] hover:border-[rgba(99,102,241,0.25)] transition-all duration-500 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-4 text-[0.75rem] uppercase tracking-[2px] font-semibold">
                      <span className="text-[var(--accent-primary)]">{role.type}</span>
                      <span className="text-[rgba(255,255,255,0.15)]">|</span>
                      <span className="text-[var(--text-dim)]">{role.location}</span>
                    </div>
                    <h3 className="font-['Space_Grotesk'] text-[2rem] tracking-[0.5px] group-hover:text-[var(--accent-primary)] transition-colors">{role.title}</h3>
                    <p className="text-[1rem] text-[var(--text-muted)] max-w-[500px]">{role.desc}</p>
                  </div>
                  <a
                    href={`mailto:Studio.A.T.S@gmail.com?subject=Application: ${role.title}`}
                    className="inline-flex items-center justify-center gap-2 py-4 px-8 text-[1rem] tracking-[1px] font-['Space_Grotesk'] rounded-xl bg-transparent text-[var(--accent-primary)] border border-[rgba(99,102,241,0.3)] hover:bg-[var(--accent-primary)] hover:text-black hover:border-[var(--accent-primary)] transition-all duration-300 text-center flex-shrink-0"
                  >
                    Apply Now
                  </a>
                </div>
            ...
            <div className="p-12 md:p-16 rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] text-center">
              ...
              <h3 className="font-['Space_Grotesk'] text-[2rem] tracking-[1px] mb-4">No Open Roles Right Now</h3>
              ...
              <a
                href="mailto:Studio.A.T.S@gmail.com?subject=Open Application"
                className="inline-flex items-center justify-center gap-3 py-4 px-10 text-[1.1rem] tracking-[1px] font-['Space_Grotesk'] rounded-xl bg-[var(--accent-primary)] text-black border border-[var(--accent-primary)] hover:bg-white hover:text-black hover:border-white transition-all duration-300 active:scale-95 shadow-lg shadow-[rgba(99,102,241,0.1)]"
              >
        ...
        {/* Why A.T.S */}
        <div className="p-12 md:p-16 rounded-2xl border border-[rgba(99,102,241,0.15)] bg-[rgba(99,102,241,0.02)]">
          <h2 className="font-['Space_Grotesk'] text-[2.5rem] tracking-[1px] uppercase mb-6">Why <span className="text-[var(--accent-primary)]">A.T.S</span>?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-[1rem] text-[var(--text-muted)] leading-[1.7]">
            <div>
              <h4 className="font-['Space_Grotesk'] text-[1.4rem] text-[var(--text-main)] tracking-[0.5px] mb-2">Remote-First</h4>
              <p>Work from anywhere. We're a distributed team that values output over office hours.</p>
            </div>
            <div>
              <h4 className="font-['Space_Grotesk'] text-[1.4rem] text-[var(--text-main)] tracking-[0.5px] mb-2">Growth Budget</h4>
              <p>Annual learning budget for courses, conferences, and tools to sharpen your edge.</p>
            </div>
            <div>
              <h4 className="font-['Space_Grotesk'] text-[1.4rem] text-[var(--text-main)] tracking-[0.5px] mb-2">Real Impact</h4>
